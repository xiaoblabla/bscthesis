...
def proxy_problem_unoptimized(n, m, weights, distances):
    ...
    for k in range(2, m+2):
        for j in range(2, n+2):
            F[j][k] = float('inf')
            for i in range(1, j): # linear scan to find the minimum value for F[j][k]
                F[j][k] = min(F[j][k], F[i][k-1] + a_tilde(i,j))
...
if __name__ == "__main__":
    ...
    result1 = proxy_problem_unoptimized(n, 0, weights, distances)
    result2 = proxy_problem_unoptimized(n, m, weights, distances)
